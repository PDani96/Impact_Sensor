...
def main():
    ...
    while True:
        ...
        #Determine if G force is dangerous to human body & take proper action (Update for martial arts applications)
        #isDanger(ts, x, y, z)

        #Write all sensor data to file AllSensorData (as you probably guessed :) )
        allData.write("x: " + str(x) + "\t" + "y: " + str(y) + "\t" + "z: " + str(z) + "\n")

        #If an increase in G-force is seen, capture temporarily. Reset after 5 seconds if no further increase is seen.
        magnitude = math.sqrt((x**2) + (y**2) + (z**2))
        
        if magnitude > displayVal:
            displayVal = magnitude
        ...
        if displayTime > 0:
            elapsedTime = time.time() - displayTime
        ...
        #print("Magnitude : %.3f G" %magnitude)

        #Debug - print all G values 
        #print("Acceleration in X-Axis : %.3f" %x)
        #print("Acceleration in Y-Axis : %.3f" %y)
        #print("Acceleration in Z-Axis : %.3f" %z)
        ...
        #print semi-colon delimited list of values for GUI (figure out how to normalize string length for better visuals)
        print ("G-Force : %6.3f;Recent Maximum : %6.3f;Personal Record : %6.3f" % (magnitude, displayVal, pr))

        #Debug - print elapsed time between recorded impacts
